...
/**
 * Creates a second store view, a `shirt_color_multi` multiselect product attribute (varchar backend)
 * with two options whose labels differ between admin and the second store, and four simple products:
 * three referencing the attribute (one blue, one yellow, one with both blue and yellow as a CSV
 * value) and one without.
 *
 * Used by ProductAttributeMultiselectOptionLabelChangeTest to verify the products feed reflects
 * option-label edits on multiselect attributes (whose backend stores a comma-separated value).
 */
class ShirtColorMultiselectProducts implements RevertibleDataFixtureInterface
{
    public const STORE_CODE = 'fixture_second_store';
    public const ATTRIBUTE_CODE = 'shirt_color_multi';

    public const SKU_BLUE = 'shirt-color-multi-product-1';
    public const SKU_YELLOW = 'shirt-color-multi-product-2';
    public const SKU_BOTH = 'shirt-color-multi-product-3';
    public const SKU_NO_ATTR = 'shirt-color-multi-product-4';
    ...
    public function revert(DataObject $data): void
    {
        ...
        foreach ([self::SKU_BLUE, self::SKU_YELLOW, self::SKU_BOTH, self::SKU_NO_ATTR] as $sku) {
            try {
    ...
    /**
     * Create the multiselect attribute (if missing) with admin labels then re-save with per-store
    ...
    private function ensureShirtColorAttribute(int $secondStoreId): array
    {
        ...
        if (!$existing->getId()) {
            ...
            $attribute->setData([
                ...
                'used_for_sort_by' => 0,
                'frontend_label' => ['Shirt Color Multi'],
                'backend_type' => 'varchar',
                'backend_model' => \Magento\Eav\Model\Entity\Attribute\Backend\ArrayBackend::class,
                'source_model' => \Magento\Eav\Model\Entity\Attribute\Source\Table::class,
                'option' => [
                    'value' => [
                        'shirt_color_multi_blue' => [self::ADMIN_LABEL_BLUE],
                        'shirt_color_multi_yellow' => [self::ADMIN_LABEL_YELLOW],
                    ],
                    'order' => [
                        'shirt_color_multi_blue' => 1,
                        'shirt_color_multi_yellow' => 2,
                    ],
                ],
            ]);
            $attribute->save();
    ...
    private function resolveOptionIds(): array
    {
        ...
        if ($blueId === null || $yellowId === null) {
            throw new \RuntimeException('Failed to resolve shirt_color_multi option ids during fixture setup.');
        }
    ...
    /**
     * Create the four simple products: blue-only, yellow-only, both (CSV), no value.
     *
    ...
    private function createProducts(int $blueOptionId, int $yellowOptionId): void
    {
        $productData = [
            [
                'sku' => self::SKU_BLUE,
                'name' => 'Shirt Color Multi Product 1',
                'value' => (string) $blueOptionId,
            ],
            [
                'sku' => self::SKU_YELLOW,
                'name' => 'Shirt Color Multi Product 2',
                'value' => (string) $yellowOptionId,
            ],
            [
                'sku' => self::SKU_BOTH,
                'name' => 'Shirt Color Multi Product 3',
                'value' => $blueOptionId . ',' . $yellowOptionId,
            ],
            [
                'sku' => self::SKU_NO_ATTR,
                'name' => 'Shirt Color Multi Product 4 (no attr)',
                'value' => null,
            ],
        ];
        ...
        foreach ($productData as $data) {
            ...
            if ($data['value'] !== null) {
                $product->setCustomAttribute(self::ATTRIBUTE_CODE, $data['value']);
            }
            $product->save();
        }
    }
